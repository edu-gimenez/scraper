...
rp_module_licence="MIT https://raw.githubusercontent.com/edu-gimenez/scraper/master/LICENSE"
...
GOPATH="$md_build" GOROOT="$goroot" "$goroot/bin/go" get -u github.com/edu-gimenez/scraper
}

function build_scraper() {
    local goroot="$(_get_goroot_golang)"
    GOPATH="$md_build" GOROOT="$goroot" "$goroot/bin/go" build github.com/edu-gimenez/scraper
}

function install_scraper() {
    md_ret_files=(
        'scraper'
        'src/github.com/edu-gimenez/scraper/LICENSE'
        'src/github.com/edu-gimenez/scraper/README.md'
        'src/github.com/edu-gimenez/scraper/hash.csv'
    )
}
...
function latest_ver_scraper() {
    wget -qO- https://api.github.com/repos/edu-gimenez/scraper/releases/latest | grep -m 1 tag_name | cut -d\" -f4
}
...
function scrape_scraper() {
    ...
    local params=()
    params+=(-ss_user "eddiegc2002")
    params+=(-ss_password "danijesus")
    ...
    params+=(-workers "1")
    params+=(-img_workers "1")
    params+=(-skip_check)
    params+=(-retries "4")
    params+=(-sleep_factor "15")
    params+=(-lang "es,en,fr")
    params+=(-img_format "png")
    params+=(-marquee_format "png")
    params+=(-convert_videos) 
    ...
    if [[ "$screenshots" -eq 1 ]]; then
        if [[ "$system" =~ ^mame-|arcade|fba|neogeo ]]; then
            params+=(-mame_img "b,fly,m,t")
        else
            params+=(-console_img "b,f,l,s")
        fi
    fi
        ...
        if [[ -n "$choice" ]]; then
            ...
            case "$choice" in
                ...
                W)
                    ...
                    ;;
                L)
                    cmd=(dialog --backtitle "$__backtitle" --inputbox "Please enter the limit the roms to processed" 10 60 "$limit")
                    limit=$("${cmd[@]}" 2>&1 >/dev/tty)
                    iniSet "limit" "$limit"
                    ;;    
                U)
                    rp_callModule "$md_id"
